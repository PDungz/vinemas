...
import 'package:vinemas_app/features/chat_bot/domain/enum/message_chat_enum.dart';
...
class ChatBotBloc extends Bloc<ChatBotEvent, ChatBotState> {
      ...
      if (fullResponse.isNotEmpty) {
        ...
        List<ChatMessage> newMessages =
            List.from(state.messages)
              ..removeWhere(
                (msg) => msg.text == (event.messageLoading ?? '...'),
              )
              ..insert(
                0,
                ChatMessage(
  ...
  Future<List<Map<String, dynamic>>> _fetchSessionMovieData(
    ...
    return await Future.wait(
      sessionMovies
          .where(
            (session) =>
                session.startDate.year == todayOnly.year &&
                session.startDate.month == todayOnly.month &&
                session.startDate.day == todayOnly.day,
          )
          .map((session) async {
            try {
              final cinema = cinemas.firstWhere(
                (element) => element.cinemaId == session.cinemaId,
              );
  ...
  Future<List<Content>> _buildChatHistory(
    SendMessageEvent event,
    List<Cinema> cinemas,
    List<SessionMovie> sessionMovies,
  ) async {
    ...
    if (event.message.medias != null && event.message.medias!.isNotEmpty) {
      ...
      final sessionMovieData = await _fetchSessionMovieData(
        cinemas,
        sessionMovies,
      );
