...
import 'package:vinemas_app/core/utils/format_datetime.dart';
...
import 'package:vinemas_app/features/about_sessions/presentation/widget/session/session_cinema_recommend_widget.dart';
...
class _SessionTitleCinemaMovieState
  ...
  Widget build(BuildContext context) {
    ...
    return BlocProvider(
      create:
          (context) => SessionBloc()..add(SessionCinemaAndSessionMovieEvent()),
                ...
                final listSessionMovie =
                    state.sessionMovie
                        ...
                        .where((element) {
                          ...
                          List<String> parts = widget.selectedTimeInterval
                              .split(' - ');
                          int startHourFilter = int.parse(
                            parts[0].replaceAll(":", ""),
                          );
                          int endHourFilter = int.parse(
                            parts[1].replaceAll(":", ""),
                          );

                          int movieStartTime = int.parse(
                            FormatDateTime.formatToHourMinute(
                              movieStartDate,
                            ).replaceAll(":", ""),
                          );
                          ...
                          bool isInTimeRange =
                              movieStartTime >= startHourFilter &&
                    ...
                    itemBuilder: (context, index) {
                      List<SessionMovie> sessionMovieCinema =
                          listSessionMovie
                              .where(
                                (element) =>
                                    cinemas[index].cinemaId ==
                                        element.cinemaId &&
                                    element.endDate.isAfter(DateTime.now()),
                              ) // Lọc bỏ suất chiếu đã qua
                              .toList()
                            ..sort(
