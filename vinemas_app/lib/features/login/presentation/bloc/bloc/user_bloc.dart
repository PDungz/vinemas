...
import 'package:vinemas_app/features/login/domain/entity/user.dart';
...
class UserBloc extends Bloc<UserEvent, UserState> {
  ...
  Future<void> registerWithEmailPassword(
    UserRegisterWithEmailPasswordEvent event,
  ...
  Future<void> loginWithEmailPassword(
    UserLoginWithEmailPasswordEvent event,
    Emitter<UserState> emit,
  ) async {
    try {
      emit(
        UserLoginWithEmailPasswordState(processStatus: ProcessStatus.loading),
      );
  ...
  Future<void> resetPassword(
    ...
    try {
      ...
      await _userUserCase.resetPassword(
        email: event.email,
        onPressed: ({required message, required status}) {
          emit(resetPasswordState(message: message, processStatus: status));
        },
      );
    } catch (e) {
      emit(
  ...
  Future<void> loginWithThirdParty(
    loginWithThirdPartyEvent event,
    Emitter<UserState> emit,
  ) async {
    try {
      ...
      if (event.method == 'google') {
        await _userUserCase.loginWithGoogle(
          ...
          onPressed: ({required message, required status}) {
            emit(
              loginWithThirdPartyState(message: message, processStatus: status),
            );
          },
        );
      }
  ...
  Future<void> sendCodeOTP(SendOtpEvent event, Emitter<UserState> emit) async {
    try {
      ...
      await _userUserCase.sendCodeOTP(
        ...
        verificationId: "",
        onPressed: ({required message, required status}) {
          emit(SendOtpState(processStatus: status, message: message));
  ...
  Future<void> logout(logoutEvent event, Emitter<UserState> emit) async {
    try {
      ...
      emit(logoutState(processStatus: ProcessStatus.failure, message: '$e'));
    }
  }
}
