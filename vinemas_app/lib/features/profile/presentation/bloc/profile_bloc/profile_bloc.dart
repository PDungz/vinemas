    ...
    try {
      await getIt<UserUseCase>().getUserInfo(
        onPressed: ({
          ...
          emit(
            (state as ProfileGetUserState).copyWith(
              user: user,
              status: status,
              message: message,
            ),
    ...
    try {
      await getIt<UserUseCase>().updateUserInfo(
        ...
        onPressed: ({required message, required status}) {
          emit(
            (state as ProfileUpdateUserState).copyWith(
              status: status,
              message: message,
            ),
          );
    ...
    try {
      ...
      if (pickedFile != null) {
        if (state is ProfileUpdateUserState) {
          emit(
            (state as ProfileUpdateUserState).copyWithImageFile(
      ...
      emit(ProfileUpdateUserState(message: "Failed to pick image"));
    }
  }
}
