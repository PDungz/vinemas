...
import 'package:vinemas_app/core/common/enum/seat_enum.dart';
...
import 'package:vinemas_app/features/about_sessions/domain/entity/session/chair_config.dart';
import 'package:vinemas_app/features/about_sessions/domain/entity/session/cinema.dart';
import 'package:vinemas_app/features/about_sessions/domain/entity/session/session_movie.dart';
...
class TicketBloc extends Bloc<TicketEvent, TicketState> {
  ...
  Future<void> getTicketMoves(
    ...
    try {
      ...
      final List<String> bookTicketId =
          payment
              .where(
                (p) => p?.ticketId != null,
              ) // Loại bỏ phần tử null hoặc ticketId null
      ...
      final List<Ticket> filteredTickets =
          ticketMovies
              .where(
                (t) => bookTicketId.contains(t.ticketId),
              ) // Kiểm tra sessionId
              .toList()
            ..sort((a, b) => b.bookedTime.compareTo(a.bookedTime));
  ...
  Future<void> getTicketDetailMoves(
    ...
    try {
      ...
      final sessionMovie = await getIt<SessionUseCase>().getSessionMovieDetail(
        sessionMovieId: event.ticket.sessionId,
      );

      final cinema = await getIt<SessionUseCase>().getCinemaDetail(
        cinemaId: sessionMovie?.cinemaId ?? '',
      );

      final movieDetail = await getIt<MovieDetailUseCase>().getMovieDetail(
        movieId: sessionMovie?.movieId ?? -1,
      );
    ...
    try {
      ...
      final List<String> bookTicketId =
          payment
              .where(
                (p) => p?.ticketId != null,
              ) // Loại bỏ phần tử null hoặc paymentId null
